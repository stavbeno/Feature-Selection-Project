...
n_exp = 20  # We run the test 100 times on different realizations, in order to evaluate the power and the error.
...
for test in tests_list:
    ...
    for ii, seed in enumerate(seed_vec):
        np.random.seed(seed)
        X, Y, beta = get_pcr_data(test=test, n=400)        #Was 1000
